...
object DataCleaning{
  def main(args: Array[String]) {
    ...
    val conf = new SparkConf().setAppName("DataCleaning").setMaster("local[2]")
    val sc = new SparkContext(conf)
    val noheader = sc.textFile("DataCleaning/src/main/resources/block_1.csv").filter(!isHeader(_))
